...
namespace Somnium
{
	namespace Graphics
	{
		...
		inline void Mesh::setup()
		{
			...
			glBufferData(GL_ARRAY_BUFFER, m_Vertices.size() * sizeof(Vertex), &m_Vertices, GL_STATIC_DRAW); //Originally GL_STATIC_DRAW

			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_EBO);
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_Indices.size() * sizeof(GLuint), &m_Indices, GL_STATIC_DRAW); //Originally GL_STATIC_DRAW

			glVertexAttribPointer(SHADER_POSITION_INDEX, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid*)offsetof(Vertex, position)/*This could just be (void*) 0 - it's like this for understanding purposes */);
